...
Rôle   : Migration passlib → bcrypt direct
...
class TestPasslibRemoved:
    ...
    def test_passlib_not_imported_in_users(self):
        """auth/users.py ne doit pas importer passlib."""
        assert "passlib" not in self._src(), (
            "auth/users.py ne doit plus importer passlib — utiliser bcrypt direct"
        )
...
class TestHashPassword:
    ...
    def test_returns_bcrypt_hash_prefix(self):
        """hash_password() génère un hash bcrypt ($2b$…)."""
