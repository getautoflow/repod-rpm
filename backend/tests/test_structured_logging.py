...
class TestMainPyMigrated:
    ...
    def test_basicconfig_removed_from_main(self):
        ...
        assert "basicConfig" not in self._main_src(), (
            ...
            "utiliser services.logging_config.setup_logging()"
        )
...
class TestLoggingConfigModule:
    ...
    def test_module_exists(self):
        ...
        assert p.exists(), "services/logging_config.py doit être créé ([T] logging)"
...
class TestRequestIdMiddlewareModule:
    ...
    def test_middleware_module_exists(self):
        ...
        assert p.exists(), "middleware/request_id.py doit être créé ([T] logging)"
    ...
    def test_python_json_logger_in_requirements(self):
        """python-json-logger doit être dans requirements.txt."""
        req = (Path(__file__).parent.parent / "requirements.txt").read_text()
        assert "python-json-logger" in req, (
            "python-json-logger doit être dans requirements.txt"
        )
