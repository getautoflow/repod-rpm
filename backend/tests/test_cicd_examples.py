...
Structure testée :
  examples/ci/
    github-upload.yml          → GitHub Actions : upload .rpm après build
    github-security-gate.yml   → GitHub Actions : bloquer PR si CVE critique
    github-sarif.yml           → GitHub Actions : export SARIF → Security tab
    gitlab-repod.yml           → GitLab CI : upload + scan + SARIF
    repod-cli.sh               → Script bash portable (tout CI/CD)

Adapté depuis repod-apt → RPM (règle R5) : .deb → .rpm
"""
...
class TestCicdFilesExist:
    ...
    def test_github_sarif_workflow_exists(self):
        """examples/ci/github-sarif.yml doit exister."""
        assert (_EXAMPLES / "github-sarif.yml").exists()
...
# ═══════════════════════════════════════════════════════════════════════════════
# 4. GitHub Actions : SARIF export
# ═══════════════════════════════════════════════════════════════════════════════

class TestGithubSarifWorkflow:
    """Vérifie le workflow SARIF → GitHub Security tab."""

    def test_references_sarif_endpoint(self):
        """Le workflow doit appeler le endpoint SARIF de repod."""
        src = _read("github-sarif.yml")
        assert "sarif" in src.lower()
        assert "/api/v1/sbom/sarif" in src or "/sarif" in src

    def test_uses_upload_sarif_action(self):
        """Le workflow doit utiliser github/codeql-action/upload-sarif."""
        src = _read("github-sarif.yml")
        assert "upload-sarif" in src, (
            "Le workflow SARIF doit envoyer le fichier à GitHub Security via upload-sarif"
        )

    def test_security_events_permission(self):
        """Le workflow doit déclarer la permission security-events: write."""
        src = _read("github-sarif.yml")
        assert "security-events" in src

    def test_saves_sarif_file(self):
        """Le workflow sauvegarde le SARIF dans un fichier .sarif ou .json."""
        src = _read("github-sarif.yml")
        assert ".sarif" in src

    def test_uses_authorization_header(self):
        """L'export SARIF est authentifié."""
        src = _read("github-sarif.yml")
        assert "Authorization" in src
...
class TestGitlabTemplate:
    ...
    def test_references_sarif_endpoint(self):
        """Le template doit exporter le SARIF."""
        src = _read("gitlab-repod.yml")
        assert "sarif" in src.lower()
...
class TestCliScript:
    ...
    def test_has_sarif_command(self):
        """Le script expose une commande sarif."""
        src = _read("repod-cli.sh")
        assert "sarif" in src.lower()
