...
Rôle   : Tests du journal d'audit immuable (append-only JSONL)
         Vérifie log(), get_recent_logs(), get_package_history()
         et la robustesse face aux fichiers corrompus.

Adapté depuis repod-apt → RPM (règle R5) : jammy → almalinux8
...
class TestAuditLog:
    ...
    def test_log_extra_fields_merged(self):
        """Les champs extra sont fusionnés dans l'entrée (R5: almalinux8 au lieu de jammy)."""
        log("UPLOAD", "alice", "SUCCESS", extra={"distribution": "almalinux8", "size": 1024})
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        entry = json.loads((Path(_TMP) / f"{today}.jsonl").read_text().splitlines()[0])
        assert entry["distribution"] == "almalinux8"
