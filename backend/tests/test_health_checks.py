...
class TestRouterImports:
    ...
    def test_security_router_imports_from_health_checks(self):
        """
        security_router.py utilise aussi get_clamav_status (endpoint /clamav/status).
        ✅ Doit importer depuis services.health_checks.
        """
        src = self._read("security_router.py")
        assert "from services.health_checks import" in src, (
            "security_router.py doit importer get_clamav_status depuis services.health_checks"
        )
    ...
    def test_security_router_no_longer_defines_private_function(self):
        ...
        (elle est maintenant dans services/health_checks.py).
        """
        src = self._read("security_router.py")
        assert "def _get_clamav_status" not in src, (
            "_get_clamav_status doit être retirée de security_router.py "
