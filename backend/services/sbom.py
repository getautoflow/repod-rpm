"""
Génération de SBOM (Software Bill of Materials) pour paquets RPM.
...
def _purl(name: str, version: str, arch: str, distribution: str) -> str:
    """Package URL (purl) spec pour les paquets RPM (RHEL/AlmaLinux/Rocky/Fedora/openSUSE)."""
...
def generate_cyclonedx(
    ...
    """
    Génère un SBOM au format CycloneDX JSON v1.5 pour paquets RPM.
    ...
    for m in manifests:
        ...
        for dep in m.get("dependencies", []):
            ...
            if dep_name:
                ext_refs.append({
                    "type": "distribution",
                    "url":  f"pkg:rpm/{pkg_distrib}/{dep_name}",
    ...
    doc = {
        ...
        "metadata": {
            ...
            "tools": [{
                "vendor":  "repod",
                "name":    "repod RPM Repository Manager",
                "version": "2.0.0",
            }],
            "component": {
                "type":    "platform",
                "name":    f"RPM Repository{f' — {distribution}' if distribution else ''}",
                "version": "1.0",
            },
        },
...
def generate_spdx(
    ...
    """
    Génère un SBOM au format SPDX JSON v2.3 (ISO/IEC 5962:2021) pour paquets RPM.
    """
    if name and version:
        manifests = []
        m = load_manifest(name, version, arch)
    ...
    for m in manifests:
        ...
        pkg = {
            ...
            "downloadLocation": f"pkg:rpm/{pkg_distrib}/{pkg_name}@{pkg_version}?arch={pkg_arch}",
            "filesAnalyzed":    False,
            "licenseConcluded": "NOASSERTION",
            "licenseDeclared":  "NOASSERTION",
            "copyrightText":    "NOASSERTION",
    ...
    return {
        ...
        "creationInfo": {
            "created":  now,
            "creators": ["Tool: repod-2.0.0", "Organization: repod RPM Repository Manager"],
